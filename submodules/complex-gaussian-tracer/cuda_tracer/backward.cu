																...
																)
{
	...
	float last_sig_re = 0.0f;
	float last_sig_im = 0.0f;
	...
	const float dL_dSr = dL_dpixel[0];
	const float dL_dSi = dL_dpixel[1];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
			...
			float sig_re = gau_signal.x;
			float sig_im = gau_signal.y;
			...
			// Signal gradients for (re, im) — simple with real alpha compositing
			float dL_dsig_re = alpha * T * dL_dSr;
			float dL_dsig_im = alpha * T * dL_dSi;

			atomicAdd(&(dL_dcolors[2 * gau_idx + 0]), dL_dsig_re);
			atomicAdd(&(dL_dcolors[2 * gau_idx + 1]), dL_dsig_im);

			// Accumulated signal for alpha gradient
			accum_rec_real = last_alpha * last_sig_re + (1.0f - last_alpha) * accum_rec_real;
			accum_rec_imag = last_alpha * last_sig_im + (1.0f - last_alpha) * accum_rec_imag;

			float dL_dalpha = (sig_re - accum_rec_real) * dL_dSr
			                + (sig_im - accum_rec_imag) * dL_dSi;
			dL_dalpha *= T;
